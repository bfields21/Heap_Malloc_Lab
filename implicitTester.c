...
{
   void *bp1, *bp2, *bp3, *bp4, *bp5, *bp6, *bp7, *bp8, *bp9, *bp10, *bp11;
   ...
   printf("----------mallocatedOurLab-----------------\n");
   printBlocks();
   printf("Blocks after bp1: malloc(0x%x),bp2: malloc(0x%x), bp3: malloc(0x%x), bp4: malloc(0x%x), bp5: malloc(0x%x), bp6: malloc(0x%x), bp7: malloc(0x%x)\n", 
   (unsigned int)bp1, (unsigned int)bp2, (unsigned int)bp3, (unsigned int)bp4, (unsigned int)bp5, (unsigned int)bp6, (unsigned int)bp7);
   ...
   mm_free(bp6);
   mm_free(bp8);
   printf("Blocks after free bp2: (0x%x), bp4: (0x%x) and bp6: (0x%x)\n", (unsigned int)bp2, (unsigned int)bp4, (unsigned int)bp6);

   printf("------------------free blocksOurLab----------------------------------\n");
   //print the block after some blocks are freed;
   printBlocks();

   bp9 = mm_malloc(0xf8); // First Fit test

   if(whichfit == FIRSTFIT)
   {
      // first_fit(GET_SIZE(bp8));
      addressCompare(bp2,bp9);
   }
   
   if(whichfit == NEXTFIT)
   {
      // next_fit(GET_SIZE(bp8));
      addressCompare(bp8, bp9);
   }

   if(whichfit == BESTFIT)
   {
      // best_fit(GET_SIZE(bp8));
       addressCompare(bp6, bp9);
   }

   bp10 =mm_malloc(0x2f8);

      if(whichfit == NEXTFIT)
   {
      // next_fit(GET_SIZE(bp8));
      addressCompare(bp4, bp10);
   }
...
void addressCompare(void *correct, void *returned)
{
   ...
   {
      if (whichfit == FIRSTFIT)
         printf("First fit placement failed.\n");
      if (whichfit == NEXTFIT)
         printf("Next fit placement failed.\n");
      if (whichfit == BESTFIT)
         printf("Best fit placement failed.\n");
