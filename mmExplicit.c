...
static void insertInFront(void *bp)
{
   ...
   // if there is another block in the list
   if (firstFree != 0)
   {
      PUT(PRED(firstFree), (unsigned int)bp);
   }
   // if there is NOT another block in the list
   else
   {
      lastFree = bp;
   }
...
static void *coalesce(void *bp)
{
   ...
   size_t beforeAlloc = GET_ALLOC(FTRP(PREV_BLKP(bp))); // holds the allocation bit of the before block
   size_t afterAlloc = GET_ALLOC(HDRP(NEXT_BLKP(bp)));  // holds the allocation bit of the after block
   size_t size = GET_SIZE(HDRP(bp));                    // holds the size of the block

   // previous/before block is allocated and the next/after is free
   ...
   // previous/before block is free and the next/after is allocated
   else if (!beforeAlloc && afterAlloc)
   {
      removeBlock(bp);
   ...
   // previous/before block is free and the next/after is free
   else if (!beforeAlloc && !afterAlloc)
   {
      removeBlock(bp);
      removeBlock(PREV_BLKP(bp));
...
static void *next_fit(size_t asize)
{
   ...
   for (bp = current; bp != 0; bp = (char *)GET(SUCC(bp)))
   {
   ...
   for (bp = firstFree; bp != current; bp = (char *)GET(SUCC(bp)))
   {
      if (GET_SIZE(HDRP(bp)) >= asize)
      {
         return bp;
...
static void *best_fit(size_t asize)
{
   ...
   for (bp = firstFree; bp != 0; bp = (char *)GET(SUCC(bp)))
   {
      if (asize <= GET_SIZE(HDRP(bp)))
      {
         ...
         else if (GET_SIZE(HDRP(bp)) < GET_SIZE(HDRP(bestFit)))
         {
            bestFit = bp;
         }
      }
...
static void removeBlock(void *bp)
{
   ...
   // If we are removing the only free block in the heap
   if (firstFree == lastFree)
   {
      // I don't think we need these two PUTs
      // PUT(SUCC(bp), 0);
      // PUT(PRED(bp), 0);
      firstFree = 0;
      lastFree = 0;
   ...
   {
      char *succPtr = (char *)GET(SUCC(bp)); // Grabs pointer to the next free block
      char *predPtr = (char *)GET(PRED(bp)); // Grabs pointer to the prev free block

      // succPtr is zero if bp is lastFree or tail
      if (succPtr == 0)
      {
         PUT(SUCC(predPtr), 0);
         lastFree = predPtr;
      ...
      // predPtr is zero it is firstFree or head
      else if (predPtr == 0)
      {
         PUT(PRED(succPtr), 0);
         firstFree = succPtr;
      }
      // succPtr and predPtr are NOT zero or bp is in the middle of the list
      else
      {
         PUT(SUCC(predPtr), (unsigned int)succPtr);
         PUT(PRED(succPtr), (unsigned int)predPtr);
      }
   }
}
