...
static void *next_fit(size_t asize)
{
   ...
   while (GET_SIZE(HDRP(pointer)) > 0)
   {
      if ((GET_ALLOC(HDRP(pointer))) == 0)
      {
         if ((GET_SIZE(HDRP(pointer))) >= asize)
         {
            return pointer;
         }
      }
      // May not reurn NULL at end of Heap!!!
      pointer = NEXT_BLKP(pointer);
   }

   pointer = heap_listp; // firstBLK
   while (GET_SIZE(HDRP(pointer)) > 0 && pointer != current)
   {
      if ((GET_ALLOC(HDRP(pointer))) == 0)
      {
         if ((GET_SIZE(HDRP(pointer))) >= asize)
...
static void *best_fit(size_t asize)
{
   ...
   void *bestFit = NULL;
   void *pointer = heap_listp; // firstBLK
   ...
   while (GET_SIZE(HDRP(pointer)) > 0)
   {
      ...
      if (GET_ALLOC(HDRP(pointer)) == 0)
      {
         ...
         if (GET_SIZE(HDRP(pointer)) >= asize)
         {
            // if bestfit hasn't been set yet
            if (bestFit == NULL)
            {
               bestFit = pointer;
            }
            // if the block is a better fit than bestfit
            else if (GET_SIZE(HDRP(pointer)) < GET_SIZE(HDRP(bestFit)))
            {
               bestFit = pointer;
            }
         }
